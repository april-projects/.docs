...
![image-20220131110349472](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/0.jpg)
...
![image-20220131142625842](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/1.jpg)

接着我们继续往下，到了` 0: invokestatic  #2                  // Method a:()I`时，需要调用方法`a()`，这时当前方法就不会继续向下运行了，而是去执行方法`a()`，那么同样的，将此方法也入栈，注意是放入到栈顶位置，`main`方法的栈帧会被压下去：

![image-20220131143641690](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/2.jpg)

这时，进入方法a之后，又继而进入到方法b，最后在进入c，因此，到达方法c的时候，我们的虚拟机栈变成了：

![image-20220131144209743](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/3.jpg)
...
![image-20220131144955668](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/4.jpg)
...
![image-20220201140516096](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/5.jpg)
...
![image-20220201141848804](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/6.jpg)
...
![image-20220201142710405](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/7.jpg)
...
![image-20220201145204505](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/8.jpg)

所以上述结果中得到的依然是两个`true`。在JDK1.7之后，稍微有一些区别，在调用`intern()`方法时，当常量池中没有对应的字符串时，不会再进行复制操作，而是将其直接修改为指向当前字符串堆中的的引用：

![image-20220201144747139](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/9.jpg)
...
![image-20220201202346882](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/10.jpg)
...
![image-20220201203157213](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/11.jpg)
...
![image-20220222084649786](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/12.jpg)
...
![image-20220222125507229](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/13.jpg)

一旦已经存在的根节点不满足存在的条件时，那么根节点与对象之间的连接将被断开。此时虽然对象1仍存在对其他对象的引用，但是由于其没有任何根节点引用，所以此对象即可被判定为不再使用。比如某个方法中的局部变量引用，在方法执行完成返回之后：

![image-20220222130350950](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/14.jpg)

这样就能很好地解决我们刚刚提到的循环引用问题，我们再来重现一下出现循环引用的情况：

![image-20220222130903349](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/15.jpg)

可以看到，对象1和对象2依然是存在循环引用的，但是只有他们各自的GC Roots断开，那么就会变成下面这样：

![image-20220222131219350](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/16.jpg)
...
![image-20220222141854678](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/17.jpg)
...
![image-20220222151708141](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/18.jpg)

那么它是如何运作的呢？

首先，所有新创建的对象，在一开始都会进入到新生代的Eden区（如果是大对象会被直接丢进老年代），在进行新生代区域的垃圾回收时，首先会对所有新生代区域的对象进行扫描，并回收那些不再使用对象：

![image-20220222153104582](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/19.jpg)
...
![image-20220222154032674](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/20.jpg)
...
![image-20220222154828416](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/21.jpg)
...
![image-20220222162536616](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/22.jpg)
...
![image-20220222205605690](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/23.jpg)
...
![image-20220222165709034](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/24.jpg)
...
![image-20220222210942507](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/25.jpg)
...
![image-20220222213208681](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/26.jpg)
...
![image-20220223104605648](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/27.jpg)
...
![image-20220223111344962](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/28.jpg)
...
![image-20220223112108949](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/29.jpg)
...
![image-20220223114019381](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/30.jpg)
...
![image-20220223123636582](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/31.jpg)

它的回收过程与CMS大体类似：

![image-20220223123557871](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/32.jpg)
...
![image-20220223130536357](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/33.jpg)

因此在JDK8时直接将本地内存作为元空间（**Metaspace**）的区域，物理内存有多大，元空间内存就可以有多大，这样永久代的空间分配问题就讲解了，所以最终它变成了这样：

![image-20220223125137512](https://tencent.cos.mobaijun.com/img/gitbook/java//JVM笔记（二）/34.jpg)
