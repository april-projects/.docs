![image-20220801205609763](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/0.png)
...
![image-20220808151202634](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/1.png)

### 树结构介绍

一棵树就像下面这样连接：

![image-20220801210920230](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/2.png)
...
![image-20220801222928422](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/3.png)
...
![image-20220801224008266](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/4.png)

并且二叉树任何结点的子树是有左右之分的，不能颠倒顺序，比如A结点左边的子树，称为左子树，右边的子树称为右子树。

二叉树有5种基本形态，分别是：

![image-20220801224513856](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/5.png)

当然，对于某些二叉树我们有特别的称呼，比如，在一棵二叉树中，所有分支结点都存在左子树和右子树，且叶子结点都在同一层：

![image-20220801231216578](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/6.png)
...
![image-20220806101322807](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/8.png)

我们优先从左边开始看，B、F、G都是A的子结点，根据上面的规律，我们将B作为左子树：

![image-20220806101841459](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/9.png)

接着继续从左往右看，由于F是B的兄弟结点，那么根据规律，F作为B的右子树：

![image-20220806102023764](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/10.png)

接着是G，G是F的兄弟结点，那么G继续作为F的右子树：

![image-20220806102123476](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/11.png)

我们接着来看第三排，依然是从左往右，C是B的子节点，所以C作为B的左子树：

![image-20220806102501769](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/12.png)

接着，D是C的兄弟节点，那么D就作为C的右子树了：

![image-20220806102619705](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/13.png)

此时还有一个H结点，它是G的子结点，所以直接作为G的左子树：

![image-20220806102802036](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/14.png)

现在只剩下最后一排了，E是D的子结点，K是H的子结点，所以最后就像这样了：

![image-20220806102932517](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/15.png)

按照规律，我们就将一棵树转换为了二叉树。当然还有一种更简单的方法，我们可以直接将所有的兄弟结点连起来（橙色横线）：

![image-20220807231603707](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/16.png)

接着擦掉所有结点除了最左边结点以外的连线：

![image-20220807231704465](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/17.png)

所有的黑色连线偏向左边，橙色连线偏向右边：

![image-20220807231922091](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/18.png)
...
![image-20220808113135783](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/19.png)

首先我们还是按照二叉树转换为树的规则，将森林中所有树转换为二叉树，接着我们只需要依次连接即可：

![image-20220808113251636](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/20.png)
...
![image-20220805231744693](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/21.png)
...
1. **由三个结点可以构造出多少种不同的二叉树？**
   ...
   *我们发现，它是非常有规律的，N每+1，项数多一项，所以我们只需要按照规律把所有情况的结果相加就行了，我们按照上面推导的结果，编写代码：*
   ...
   ![image-20220808121124094](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/22.png)
...
3. **深度为h的满m叉树的第k层有多少个结点？**
   ...
   *这道题只是看着复杂，但是实际上我们把之前推导都公式带进来就行了。但是注意，难点在于，这道题给的是满m叉树，而不是满二叉树，满二叉树根据性质一我们已经知道：*
   ...
   ![image-20220808131305843](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/23.png)
...
![image-20220806110546789](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/25.png)
...
![image-20220723171648380](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/26.png)

而二叉树也可以使用这样的链式存储形式，只不过现在一个结点需要存放一个指向左子树的指针和一个指向右子树的指针了：

![image-20220806111610082](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/27.png)
...
![image-20220806113156166](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/29.png)
...
![image-20220806171459056](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/30.png)

前序遍历是一种勇往直前的态度，走到哪就遍历到那里，先走左边再走右边，比如上面的这个图，首先会从根节点开始：

![image-20220806171431845](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/31.png)
...
![image-20220806173227580](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/32.png)
...
![image-20220806215229564](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/34.png)

当遍历到D结点时，没有左子树了，此时将栈顶结点D出栈，发现没有右节点，继续出栈，得到B结点，接着得到当前结点的右孩子E结点，然后重复上述步骤：

![image-20220806220752941](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/35.png)

接着发现E也没有左子树了，同样的，又开始出栈，此时E没有右子树，接着看A，A有右子树，所以继续从C开始，重复上述步骤：

![image-20220806221147022](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/36.png)

由于C之后没有左子树，那么就出栈获取右子树，此时得到结点F，继续重复上述步骤：

![image-20220806221239705](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/37.png)
...
![image-20220806230603967](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/38.png)
...
![image-20220806231752418](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/39.png)
...
![image-20220806233407910](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/40.png)
...
![image-20220806234428922](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/41.png)
...
![image-20220807205135936](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/42.png)
...
![image-20220807211522409](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/43.png)

接着开始不断重复上面的步骤，首先是将队首元素出队，打印A，然后将A的左右孩子依次入队：

![image-20220807211631110](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/44.png)

现在队列中有B、C两个结点，继续重复上述操作，B先出队，打印B，然后将B的左右孩子依次入队：

![image-20220807211723776](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/45.png)

现在队列中有C、D、E这三个结点，继续重复，C出队并打印，然后将F入队：

![image-20220807211800852](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/46.png)
...
![image-20220807215630429](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/47.png)
...
![image-20220814145531577](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/48.png)

我们就可以将其进行线索化，首先还是按照前序遍历的顺序依次寻找：

![image-20220814150731326](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/49.png)
...
![image-20220814151342130](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/50.png)
...
![image-20220814152008732](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/51.png)

接着我们来看结点E，这个结点只有一个右孩子，没有左孩子，左孩子指针为NULL，我们也可以将其线索化：

![image-20220814152117861](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/52.png)

最后，整棵二叉树完成线索化之后，除了遍历顺序的最后一个结点没有后续之外，其他为NULL的指针都被利用起来了：

![image-20220814152341658](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/53.png)

我们可以发现，在利用上那些为NULL的指针之后，当我们再次进行前序遍历时，我们不需要再借助栈了，而是可以一路向前。

这里我们弄一个简单一点的线索化二叉树，来尝试对其进行遍历：

![image-20220814152703468](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/54.png)
...
![image-20220814154539765](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/55.png)
...
![image-20220814161529021](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/56.png)
...
![image-20220814162606692](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/57.png)
...
![image-20220814191444130](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/58.png)
...
![image-20220815094708456](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/59.png)

比18小的结点在左边，大的在右边，满足二叉查找树的性质。接着是7、15、22：

![image-20220815094823646](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/60.png)
...
![image-20220815095915453](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/61.png)
...
![image-20220815104036598](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/62.png)

而第二种情况，就有点麻烦了，因为有一个孩子，就像一个拖油瓶一样，你离开了还不行，你还得对他负责才可以。当移除后，需要将孩子结点连接上去：

![image-20220815104553978](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/63.png)
...
![image-20220815110311555](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/64.png)
...
![image-20220815113242191](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/65.png)
...
![image-20220815113705827](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/66.png)
...
![image-20220815210652973](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/67.png)

通过计算平衡因子，我们就可以快速得到是否出现失衡的情况。比如下面的这棵二叉树，正在执行插入操作：

![image-20220815115219250](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/68.png)

可以看到，当插入之后，不再满足平衡二叉树的定义时，就出现了失衡的情况，而对于这种失衡情况，为了继续保持平衡状态，我们就需要进行处理了。我们可能会遇到以下几种情况导致失衡：

![image-20220815115836604](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/69.png)
...
1. **LL型调整**（右旋）
   ...
   ![image-20220815211641144](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/70.png)

   首先我们来看这种情况，这是典型的LL型失衡，为了能够保证二叉树的平衡，我们需要将其进行**旋转**来维持平衡，去纠正最小不平衡子树即可。那么怎么进行旋转呢？对于LL型失衡，我们只需要进行右旋操作，首先我们先找到最小不平衡子树，注意是最小的那一个：

   ![image-20220815212552176](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/71.png)

   可以看到根结点的平衡因子是2，是目前最小的出现不平衡的点，所以说从根结点开始向左的三个结点需要进行右旋操作，右旋需要将这三个结点中间的结点作为新的根结点，而其他两个结点现在变成左右子树：

   ![image-20220815213222964](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/72.png)
...
2. **RR型调整**（左旋）
   ...
   ![image-20220815214026710](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/73.png)

   操作和上面是一样的，只不过现在反过来了而已：

   ![image-20220815214408651](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/74.png)
...
3. **RL型调整**（先右旋，再左旋）
   ...
   ![image-20220815214859501](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/75.png)

   可以看到现在的形状是一个回旋镖形状的，先右后左的一个状态，也就是RL型，针对于这种情况，我们需要先进行右旋操作，注意这里的右旋操作针对的是后两个结点：

   ![image-20220815215929303](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/76.png)
...
4. **LR型调整**（先左旋，再右旋）
   ...
   ![image-20220815220609357](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/77.png)

   形状是先向左再向右，这就是典型的LR型了，我们同样需要对其进行两次旋转：

   ![image-20220815221349044](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/78.png)
...
![image-20220815222810537](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/79.png)
...
![image-20220816104917851](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/80.png)

首先这棵红黑树只有一个根结点，因为根结点必须是黑色，所以说直接变成黑色。现在我们要插入一个新的结点了，所有新插入的结点，默认情况下都是红色：

![image-20220816105119178](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/81.png)

所以新来的结点7根据规则就直接放到11的左边就行了，然后注意7的左右两边都是NULL，那么默认都是黑色，这里就不画出来了。同样的，我们往右边也来一个：

![image-20220816105553070](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/82.png)

现在我们继续插入一个结点：

![image-20220816105656320](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/83.png)
...
![image-20220816113259643](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/84.png)
...
![image-20220816113339344](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/85.png)

接着我们继续插入结点：

![image-20220816113939172](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/86.png)

此时又来了一个插在4左边的结点，同样是连续红色，我们需要进行变色才可以讲解问题，但是我们发现，如果变色的话，那么从11开始到所有NIL结点经历的黑色结点数量就不对了：

![image-20220816114245996](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/87.png)

所以说对于这种**父结点为红色，父结点的兄弟结点为黑色**（NIL视为黑色）的情况，变色无法解决问题了，那么我们只能考虑旋转了，旋转规则和我们之前讲解的平衡二叉树是一样的，这实际上是一种LL型失衡：

![image-20220816115015892](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/88.png)

同样的，如果遇到了LR型失衡，跟前面一样，先左旋在右旋，然后进行变色即可：

![image-20220816115924938](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/89.png)
...
![image-20220817102503116](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/90.png)
...
![image-20220817105907362](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/91.png)
...
![image-20220817110243376](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/92.png)
...
![image-20220817110803123](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/93.png)

就像爱情一样，两个人的世界容不下第三者，如果来了第三者，那么最后的结果大概率就是各自分道扬镳。接着我们继续插入4、5看看会发生什么，注意插入还是按照小的走左边，大的走右边的原则，跟我们之前的二叉查找树是一样的：

![image-20220817111405624](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/94.png)

此时4、5来到了右边，现在右边这个结点又被撑爆了，所以说需要按照上面的规则，继续进行分割：

![image-20220817111556446](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/95.png)
...
![image-20220817111943543](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/96.png)

此时右下角结点又被挤爆了，右下角真是多灾多难啊，那么依然按照我们之前的操作进行分裂：

![image-20220817112213868](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/97.png)

我们发现当新的分割值上升之后最上面的结点又被挤爆了，此时我们需要继续分裂：

![image-20220817112401155](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/98.png)
...
![image-20220817114440027](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/99.png)

删除后，依然满足B树的性质，所以说什么都不管用：

![image-20220817114541675](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/100.png)

此时我们接着去删除15结点：

![image-20220817114722079](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/101.png)

删除后，现在结点中只有14了，不满足B树的性质：除根结点和叶子结点外，其它每个结点至少有⌈m/2⌉个孩子，同理键值数量至少有⌈m/2⌉-1个，现在只有一个肯定是不行的。此时我们需向兄弟（注意只能找左右两边的兄弟）借一个过来：

![image-20220817114956686](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/102.png)

此时我们继续删掉17，但是兄弟已经没办法再借给我们一个元素了，此时只能采取方案二，合并兄弟节点与分割键。这里我们就合并左边的这个兄弟吧：

![image-20220817120014656](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/103.png)

![image-20220817120058865](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/104.png)

现在他们三个又合并回去了，这下总满足了吧？但是我们发现，父结点此时只有一个元素了，又出问题了。同样的，还是先去找兄弟结点借一个，但是兄弟结点也借不了了，此时继续采取我们的方案二，合并：

![image-20220817120402123](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/105.png)

OK，这样才算是满足了B树的性质，现在我们继续删除4结点：

![image-20220817120835776](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/106.png)

这种情况会导致失去分割值，那么我们得找一个新的分割值才行，这里取左边最大的：

![image-20220817121020793](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/107.png)

不过此时虽然解决了分割值的问题，但是新的问题来了，左边结点不满足性质了，元素数量低于限制，于是需要找兄弟结点借，但是没得借了，兄弟也没有多的可以借了所以被迫合并了：

![image-20220817121250186](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/108.png)
...
>
> ![image-20220817123042186](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/109.png)
>
> 此时我们将所有红色节点上移到与父结点同一高度，
>
> ![image-20220817123537220](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/110.png)
>
> 还是没看出来？没关系，我们来挨个画个框：
>
> ![image-20220817123455865](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/111.png)
>
> woc，这不就是B树吗？没错，**红黑树** 和 **4阶B树**（2-3-4树）具有等价性，其中黑色结点就是中间的（黑色结点一定是父结点)，红色结点分别位于两边，通过将黑色结点与它的红色子节点融合在一起，形成1个B树节点，最后就像这样：
>
> ![image-20220817153152790](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/112.png)
>
> 你会发现，红黑树的黑色节点个数总是与4阶B树的节点数相等。我们可以对比一下之前的红黑树插入和4阶B树的插入，比如现在我们想要插入一个新的14结点进来：
>
> ![image-20220817153955759](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/113.png)
>
> 经过变色，最后得到如下的红黑树，此时又出现两个红色结点连续，因为父结点的兄弟结点依然是红色，继续变色：
>
> ![image-20220817154655210](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/114.png)
>
> 最后因为根结点必须是黑色，所以说将60变为黑色，这样就插入成功了：
>
> ![image-20220817154751660](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/115.png)
>
> 我们再来看看与其等价的B树插入14后会怎么样：
>
> ![image-20220817154838567](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/116.png)
>
> 由于B树的左边被挤爆了，所以说需要分裂，因为是偶数个，需要选择中间偏右的那个数作为分割值，也就是25：
>
> ![image-20220817160036666](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/117.png)
>
> 分裂后，分割值上升，又把父结点给挤爆了，所以说需要继续分裂：
>
> ![image-20220817160244020](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/118.png)
>
> 现在就变成了这样，我们来对比一下红黑树：
>
> ![image-20220817160427011](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/119.png)
>
...
![image-20220817163343975](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/120.png)
...
![image-20220817163816562](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/121.png)
...
![image-20220817170310064](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/122.png)
...
![image-20220817171759738](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/123.png)

首先我们选择两棵权值最小的树作为一颗新的树的左右子树，左右顺序不重要（因为哈夫曼编码不唯一，后面会说），得到的树根结点权值为这两个结点之和：

![image-20220817172343786](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/124.png)

接着，我们需要将这这棵树放回到森林中，重复上面的操作，继续选择两个最小的出来组成一颗新的树，此时得到：

![image-20220817172640686](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/125.png)

继续重复上述操作，直到森林里面只剩下一棵树为止：

![image-20220817172737480](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/126.png)
...
![image-20220817173559604](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/127.png)
...
![image-20220817174835425](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/128.png)

比如一开始4和9排在队列中，这时又来了个7，那么由于7比4大，所以说可以插队，直接排到4的前面去，但是由于9比7大，所以说不能再往前插队了：

![image-20220817174921980](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/129.png)
...
![image-20220817180127650](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/130.png)
...
![image-20220817180820954](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/131.png)
...
![image-20220817184746630](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/132.png)
...
![image-20220818104754776](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/133.png)

因为完全二叉树比较适合使用数组才存储（因为是按序的）所以说一般堆都是以数组形式存放：

![image-20220818110224673](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/134.png)

那么它是怎么运作的呢？比如现在我们想要往堆中插入一个新的元素8，那么：

![image-20220818110450863](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/135.png)
...
![image-20220818110835798](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/136.png)

同样的，数组的形式的话，我们就行先计算出它的父结点，然后进行交换即可：

![image-20220818111156209](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/137.png)

当然，还没完，我们还需要继续向上比较，直到稳定为止，此时7依然是小于8的，所以说需要继续交换：

![image-20220818111311322](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/138.png)

现在满足性质了，堆化结束，可以看到最大的元素被排到了最前面，这不就是我们前面的优先级队列吗。

现在我们来试试看删除队首元素，也就相当于出队操作，删除最顶上的元素：

![image-20220818111840303](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/139.png)

现在需要删除最顶上的元素但是我们需要保证删除之后依然是一棵完全二叉树，所以说我们先把排在最后面的拿上来顶替一下：

![image-20220818111959046](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/140.png)

![image-20220818112109066](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/141.png)

接着我们需要按照与插入相反的方向，从上往下进行堆化操作，规则是一样的，遇到大的就交换，直到不是为止：

![image-20220818112222696](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/142.png)
...
![image-20220818120554099](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/143.png)
...
![image-20220818120633714](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/144.png)
...
> ![img](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/145.jpg)
>
...
> ![img](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/146.jpg)
>
...
> ![img](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/147.jpg)
>
...
> ![img](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/148.jpg)
>
> 输入：root = [2,1,3]
> 输出：true
...
> ![img](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/149.jpg)
>
...
![image-20220817224437688](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/150.png)

不可能啊，我们的逻辑判断没有问题的，我们的算法不可能被卡的啊？（这跟我当时打ACM一样的感觉，我这天衣无缝的算法不可能错的啊，哪个老六测试用例给我卡了）这其实是因为我们没有考虑到右子树中左子树比根结点值还要小的情况：

![image-20220817224830911](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/151.png)
...
> ![img](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/152.jpg)
>
> 输入：root = [1,2,3]
> 输出：25
> 解释：
...
> ![img](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/153.jpg)
>
> 输入：root = [4,9,0,5,1]
> 输出：1026
> 解释：
...
> ![img](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/154.jpg)
>
> 输入：root = [1,2,3]
> 输出：6
...
> ![img](https://tencent.cos.mobaijun.com/img/gitbook/Java/算法/数据结构与算法（二）/155.jpg)
>
